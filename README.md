...
### LanguageTool rate-limit troubleshooting

If you see `language_tool_python.exceptions.RateLimitError`, the public LanguageTool API has throttled your requests.

Use one of these options:

```bash
# Option 1 (recommended): auto mode (default) with runtime fallback to local
python stt_grammar_pipeline.py --dataset-dir /home/you/datasets/LJSpeech-1.1 --grammar-mode auto

# Option 2: skip grammar correction entirely
python stt_grammar_pipeline.py --dataset-dir /home/you/datasets/LJSpeech-1.1 --no-grammar
```

When rate limiting happens, the script now falls back gracefully (local tool in `auto` mode, or raw ASR text for that file if fallback is unavailable).
