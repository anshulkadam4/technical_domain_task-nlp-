...
def safe_grammar_correct(
    tool,
    text: str,
    language: str,
    grammar_mode: str,
) -> tuple[str, object, Optional[str]]:
    """Apply grammar correction with runtime fallback on API rate-limit/errors.

    Returns corrected_text, possibly-updated tool, and optional error tag.
    """
    if tool is None:
        return text, tool, None

    try:
        return grammar_correct(tool, text), tool, None
    except Exception as exc:
        exc_name = exc.__class__.__name__
        is_rate_limited = exc_name == "RateLimitError" or "rate limit" in str(exc).lower()

        if grammar_mode == "auto" and is_rate_limited:
            warnings.warn(
                "LanguageTool public API rate limit reached; falling back to local grammar tool.",
                RuntimeWarning,
            )
            try:
                local_tool = init_grammar_tool(language, mode="local")
                return grammar_correct(local_tool, text), local_tool, "public_rate_limited_fallback_local"
            except Exception as local_exc:
                warnings.warn(
                    f"Local grammar fallback failed ({local_exc}); using raw ASR output.",
                    RuntimeWarning,
                )
                return text, tool, "grammar_fallback_failed"

        if grammar_mode == "public" and is_rate_limited:
            warnings.warn(
                "LanguageTool public API rate limit reached; using raw ASR output for this file.",
                RuntimeWarning,
            )
            return text, tool, "public_rate_limited_raw_used"

        warnings.warn(
            f"Grammar correction failed ({exc}); using raw ASR output for this file.",
            RuntimeWarning,
        )
        return text, tool, "grammar_failed_raw_used"
...
def main() -> None:
    ...
    for wav_file in tqdm(wav_files, desc="Processing audio"):
        ...
        corrected_text, grammar_tool, grammar_error = safe_grammar_correct(
            grammar_tool,
            raw_text,
